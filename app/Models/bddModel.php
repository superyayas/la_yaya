...
class yayaBD{
    ...
    public function actualizarCliente($id,$nombre,$apellidos,$telefono,$email,$contrasena){
        $consulta = "UPDATE `usuarios` SET `nombre` =?,`apellidos` =?,`telefono` =?,email` =?,`contrasena` =? WHERE `id` =?";
        $actualizar = yayaBD::consultaInsercionBD($consulta,$nombre,$apellidos,$telefono,$email,$contrasena,$id);
        return $actualizar;
    }
